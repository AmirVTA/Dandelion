...
function is_realm(msg)
  local var = false
  local realms = 'realms'
  local data = load_data(_config.groups.data)
...
-- Custom fuctions start
--Check if this chat is a user or not
function is_user(msg)
  local var = false
  local data = load_data(_config.users.data)
  local users = 'users'
  local userid = msg.from.id
  if data[tostring(users)] then
    if data[tostring(users)][tostring(userid)] then
		if msg.to.type == 'user' then
			var = true
		end
    end
...
function is_log_user(msg)
  local var = false
  local data = load_data(_config.users.data)
  local GBan_log = 'GBan_log'
  if data[tostring(GBan_log)] then
	if data[tostring(GBan_log)][tostring(msg.from.id)] then
		if msg.to.type == 'user' then
			var = true
		end
		return var
...
function saveuserlog(user, logtxt)
local text = (os.date("[ %c ]=>  "..logtxt.."\n \n"))
local file = io.open("./users/logs/"..group.."log.txt", "a")
file:write(text)
file:close()
end
-- Custom function end
